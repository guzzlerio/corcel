...
func (instance HTMLReporter) Generate(output statistics.AggregatorSnapShot) {
	...
	layout := mustache.Render(string(summaryLayout), map[string]interface{}{
		...
		"min_latency":        fmt.Sprintf("%.0f ms", executionSummary.ResponseTime.Min),
		"mean_latency":       fmt.Sprintf("%.0f ms", executionSummary.ResponseTime.Mean),
		"max_latency":        fmt.Sprintf("%.0f ms", executionSummary.ResponseTime.Max),
